...
@section('Konten')
    ...
    <body>
        <h2>{{ isset($konfigurasiservice->id) ? 'Update' : 'Create' }} Konfigurasi Layanan</h2>
        ...
        <form action="{{ route('konfigurasiservice.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div>
                <label for="nav">Navigasi:</label>
                <input type="text" name="nav" id="nav"
                    value="{{ old('nav', $konfigurasiservice->nav) }}">
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="title" id="title"
                        value="{{ old('title', $konfigurasiservice->title) }}">
                </div>

                <div class="form-group">
                    <label for="description">Deskripsi:</label>
                    <input type="text" name="description" id="description"
                        value="{{ old('description', $konfigurasiservice->description) }}">
                </div>
            </div>

            <button type="submit" class="btn">{{ isset($konfigurasiservice->id) ? 'Update' : 'Create' }}</button>
        </form>

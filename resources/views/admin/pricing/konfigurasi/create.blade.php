...
@section('Konten')
    ...
    <body>
        <h2>{{ isset($konfigurasipricing->id) ? 'Update' : 'Create' }} Konfigurasi Harga</h2>
        ...
        <form action="{{ route('konfigurasipricing.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="nav" id="nav"
                        value="{{ old('nav', $konfigurasipricing->nav) }}">
                </div>

                <div class="form-group">
                    <label for="title">Judul:</label>
                    <input type="text" name="title" id="title"
                        value="{{ old('title', $konfigurasipricing->title) }}">
                </div>
            </div>
                ...
                <div>
                    ...
                    <input type="text" name="overview" id="overview"
                        value="{{ old('overview', $konfigurasipricing->overview) }}">
                </div>

            <button type="submit" class="btn">{{ isset($konfigurasipricing->id) ? 'Update' : 'Create' }}</button>
        </form>

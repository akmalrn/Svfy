...
@section('Konten')
    ...
    <body>
        <h2>{{ isset($konfigurasiteam->id) ? 'Update' : 'Create' }} Konfigurasi Team</h2>
        ...
        <form action="{{ route('teamkonfigurasi.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="nav" id="nav"
                        value="{{ old('nav', $konfigurasiteam->nav) }}">
                </div>

                <div class="form-group">
                    <label for="title">Judul:</label>
                    <input type="text" name="title" id="title"
                        value="{{ old('title', $konfigurasiteam->title) }}">
                </div>
            </div>
                ...
                <div>
                    ...
                    <input type="text" name="overview" id="overview"
                        value="{{ old('overview', $konfigurasiteam->overview) }}">
                </div>

            <button type="submit" class="btn">{{ isset($konfigurasiteam->id) ? 'Update' : 'Create' }}</button>
        </form>

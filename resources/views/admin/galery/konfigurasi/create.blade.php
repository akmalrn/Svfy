...
@section('Konten')
    ...
    <body>
        <h2>{{ isset($konfigurasigallery->id) ? 'Update' : 'Create' }} Konfigurasi Galeri</h2>
        ...
        <form action="{{ route('konfigurasigallery.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-group"> 
                <label for="nav">Navigasi:</label>
                <input type="text" name="nav" id="nav"
                    value="{{ old('nav', $konfigurasigallery->nav) }}">
            </div>

                <div>
                    <label for="title">Judul:</label>
                    <input type="text" name="title" id="title"
                        value="{{ old('title', $konfigurasigallery->title) }}">
                </div>

            <button type="submit" class="btn">{{ isset($konfigurasigallery->id) ? 'Update' : 'Create' }}</button>
        </form>
